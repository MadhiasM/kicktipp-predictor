...
def get_communities(browser):
    c = [[cl['href'] for cl in cd.find_all('a', href=True)] for cd in browser.find(id=CONTENT).find(class_='pagecontent').find(class_='menu').find_all(class_='level0')]
    cm = []
    for i in c:
        cm.append(i[0].strip('/'))
    return cm
...
def get_matchdays(browser):
    return [m['href'].split('=')[-1] for m in browser.find(id=CONTENT).find(class_='dropdowncontent').find_all('a', href=True)]
...
def get_deadline(browser: RoboBrowser): # Minutes
    ...
    deadline = int(rules.find('h2', string = lambda t: DEADLINE_RULE  in t.string).text.split(' ')[1])
    return deadline
    ...
    for community in communities:
        ...
        for matchday in matchdays:
            ...
            for match in matches:
                ...
                try:
                    # Home, draw, away odds
                    match_odds = [float(o) for o in match[4].get_text().strip('Quote: ' ).replace('/','').split('  ')]
                except IndexError as err: # Matchdays where no odds are available at all
                    #print(f'No odds found for {match_round} in match {home} vs. {away} on {match_time}. Skipped.')
                    match_odds = "N/A"
                    match_bet = ('-','-')
                except ValueError as err: # Matchdays where odds are available only partially
                    match_odds = "N/A"
                    match_bet = ('-','-')
